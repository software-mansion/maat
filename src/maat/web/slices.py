...
def make_slices(reports: list[ReportInfo]) -> list[Slice]:
    ...
    if latest_nightly and latest_stable:
        push_slice("Nightly vs Stable", [*latest_nightly, *latest_stable])
    ...
    push_slice(f"Last {len(last_n_scarbs)} Scarbs", last_n_scarbs, default=True)
    ...
    push_slice(f"Last {len(last_n_foundries)} Foundries", last_n_foundries)
